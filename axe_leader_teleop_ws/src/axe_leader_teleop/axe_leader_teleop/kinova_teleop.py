...
Linear axes: leader frame +x=fwd, +y=left, +z=up → Kinova base via movo_linear_axis_map in
  movoHead_ws/.../home_joints.yaml (homing_pose_name + arm_namespace). Optional movo_linear_axis_map param overrides.
...
class KinovaTeleop(Node):
    def __init__(self):
        ...
        # Empty → home_joints.yaml movo_linear_axis_map[homing_pose_name][arm_namespace]
        self.declare_parameter("movo_linear_axis_map", "")
        self.declare_parameter("teleop_axis", "")  # deprecated; used only if movo_linear_axis_map empty
        ...
        ta_override = mo or legacy
        if ta_override:
            self._axis_spec_str = ta_override
        else:
            self._axis_spec_str = movo_linear_axis_map_for_arm(self.arm_namespace)
        ...
        self.get_logger().info(
            ...
            f"movo_linear_axis_map={self._axis_spec_str} (home_joints.yaml: {_yp})"
        )
        self.get_logger().info(
    ...
    def _map_linear(self, v_leader: list[float]) -> list[float]:
        """Leader (+x fwd, +y left, +z up) → Kinova base linear, via home_joints movo_linear_axis_map."""
